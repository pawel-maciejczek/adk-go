...
func (w *consoleLauncher) initTelemetry(ctx context.Context, config *launcher.Config) (telemetry.Telemetry, error) {
	...
	telemetry, err := telemetry.NewWithOptions(ctx, config.TelemetryOptions...)
	if err != nil {
		return nil, err
	}
	log.Printf("Telemetry - initialized.")
	telemetry.SetGlobalProviders()
	log.Printf("Telemetry - installed global otel providers.")
